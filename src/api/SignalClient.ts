...
export class SignalClient {
  ...
  sendMuteTrack(trackSid: string, muted: boolean) {
    this.log.debug('sending Mute Track req to server', { ...this.logContext,trackSid:trackSid, muted:muted});
    return this.sendRequest({
      case: 'mute',
  ...
  sendUpdateTrackSettings(settings: UpdateTrackSettings) {
    this.log.debug('sending Update Track setting req to server', { ...this.logContext,settings:settings});
    this.sendRequest({
      case: 'trackSetting',
      value: settings,
    });
  ...
  sendUpdateSubscription(sub: UpdateSubscription) {
    this.log.debug('sending Update Subscription req to server', { ...this.logContext,sub:sub});
    return this.sendRequest({
      case: 'subscription',
      value: sub,
    });
  ...
  sendSyncState(sync: SyncState) {
    this.log.debug('sending sync state req to server', { ...this.logContext,sync:sync});
    return this.sendRequest({
      case: 'syncState',
      value: sync,
    });
  ...
  sendUpdateVideoLayers(trackSid: string, layers: VideoLayer[]) {
    this.log.debug('sending update video layer req to server', { ...this.logContext,trackSid:trackSid, layers:layers });
    return this.sendRequest({
      case: 'updateLayers',
  ...
  sendUpdateSubscriptionPermissions(allParticipants: boolean, trackPermissions: TrackPermission[]) {
    this.log.debug('sending update subscription permission req to server', { ...this.logContext, allParticipants:allParticipants, trackPermissions:trackPermissions });
    return this.sendRequest({
  ...
  sendSimulateScenario(scenario: SimulateScenario) {
    this.log.debug('sending simulate scenario req to server', { ...this.logContext, scenario:scenario });
    return this.sendRequest({
      case: 'simulate',
      value: scenario,
    });
  ...
  async sendRequest(message: SignalMessage, fromQueue: boolean = false) {
    ...
    this.log.debug('sending request  to server ', { ...this.logContext, message:message });
  ...
  private handleSignalResponse(res: SignalResponse) {
    ...
    if (msg.case === 'answer') {
      this.log.debug(`the answer response from server is: `, { ...this.logContext, response:res });
      const sd = fromProtoSessionDescription(msg.value);
      if (this.onAnswer) {
        this.onAnswer(sd);
      }
    } else if (msg.case === 'offer') {
      this.log.debug(`the offer response from server is: `, { ...this.logContext, response:res });
      const sd = fromProtoSessionDescription(msg.value);
      if (this.onOffer) {
        this.onOffer(sd);
      }
    } else if (msg.case === 'trickle') {
      this.log.debug(`the trickle response from server is: `, { ...this.logContext, response:res });
      const candidate: RTCIceCandidateInit = JSON.parse(msg.value.candidateInit!);
      if (this.onTrickle) {
        this.onTrickle(candidate, msg.value.target);
      }
    } else if (msg.case === 'update') {
      this.log.debug(`the update response from server is: `, { ...this.logContext, response:res });
      if (this.onParticipantUpdate) {
        this.onParticipantUpdate(msg.value.participants ?? []);
      }
    } else if (msg.case === 'trackPublished') {
      this.log.debug(`the trackPublished response from server is: `, { ...this.logContext, response:res });
      if (this.onLocalTrackPublished) {
        this.onLocalTrackPublished(msg.value);
      }
    } else if (msg.case === 'speakersChanged') {
      this.log.debug(`the speakersChanged response from server is: `, { ...this.logContext, response:res });
      if (this.onSpeakersChanged) {
        this.onSpeakersChanged(msg.value.speakers ?? []);
      }
    } else if (msg.case === 'leave') {
      this.log.debug(`the leave response from server is: `, { ...this.logContext, response:res });
      if (this.onLeave) {
        this.onLeave(msg.value);
      }
    } else if (msg.case === 'mute') {
      this.log.debug(`the mute response from server is: `, { ...this.logContext, response:res });
      if (this.onRemoteMuteChanged) {
        this.onRemoteMuteChanged(msg.value.sid, msg.value.muted);
      }
    } else if (msg.case === 'roomUpdate') {
      this.log.debug(`the roomUpdate response from server is: `, { ...this.logContext, response:res });
      if (this.onRoomUpdate && msg.value.room) {
        this.onRoomUpdate(msg.value.room);
      }
    } else if (msg.case === 'connectionQuality') {
      this.log.debug(`the connectionQuality response from server is: `, { ...this.logContext, response:res });
      if (this.onConnectionQuality) {
        this.onConnectionQuality(msg.value);
      }
    } else if (msg.case === 'streamStateUpdate') {
      this.log.debug(`the streamStateUpdate response from server is: `, { ...this.logContext, response:res });
      if (this.onStreamStateUpdate) {
        this.onStreamStateUpdate(msg.value);
      }
    } else if (msg.case === 'subscribedQualityUpdate') {
      this.log.debug('the subscribedQualityUpdate response from server is: ', { ...this.logContext, response:res });
      if (this.onSubscribedQualityUpdate) {
        this.onSubscribedQualityUpdate(msg.value);
      }
    } else if (msg.case === 'subscriptionPermissionUpdate') {
      this.log.debug('the subscriptionPermissionUpdate response from server is: ', { ...this.logContext, response:res });
      if (this.onSubscriptionPermissionUpdate) {
        this.onSubscriptionPermissionUpdate(msg.value);
      }
    } else if (msg.case === 'refreshToken') {
      this.log.debug('the refreshToken response from server is: ', { ...this.logContext, response:res });
      if (this.onTokenRefresh) {
        this.onTokenRefresh(msg.value);
      }
    } else if (msg.case === 'trackUnpublished') {
      this.log.debug('the trackUnpublished response from server is:', { ...this.logContext, response:res });
