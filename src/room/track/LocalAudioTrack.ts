...
import { AudioTrackFeature } from '../../proto/livekit_models_pb';
...
export default class LocalAudioTrack extends LocalTrack<Track.Kind.Audio> {
  ...
  protected monitorSender = async () => {
    ...
    try {
      ...
      this.log.error('could not get audio sender stats', {
        ...this.logContext,
        error: e,
      });
      return;
  ...
  async setProcessor(
    ...
    try {
      ...
      this.log.debug(
        `setting up audio processor ${processor.name}`,
        this.logContext
      );
