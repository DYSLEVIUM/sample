...
helpFunction() {
   echo "Usage: $0 --room <room> --livekit_url <livekit_url> --server_api_url <server_api_url> --idcs_token <idcs_token> [--front_token <front_token>] [--rear_token <rear_token>] [--left_token <left_token>] [--right_token <right_token>]"
   echo -e "\t--room: Room to connect"
   echo -e "\t--livekit_url: Livekit server URL"
   echo -e "\t--server_api_url: Server API URL"
   echo -e "\t--idcs_token: Oracle Identity Cloud Service token"
...
if [ -z "$room" ] || [ -z "$livekit_url" ] || [ -z "$server_api_url" ] || [ -z "$idcs_token" ]; then
...
# Now you can use $front_token, $rear_token, $left_token, $right_token, and $idcs_token as needed in the script.
